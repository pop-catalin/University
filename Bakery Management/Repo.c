...
int find(MaterialRepo* v, char name[], char supplier[], char date[]) {
...
int add(MaterialRepo* v, Material m) {
	if (find(v, m.name, m.supplier, m.date) != -1)
		changeQuantity(&v->materials[find(v, m.name, m.supplier, m.date)], m.quantity);
	else {
...
int del(MaterialRepo* v, Material m) {
	if (find(v, m.name, m.supplier, m.date) == -1)
		return 0;
	for(int i = find(v, m.name, m.supplier, m.date); i < v->length; ++i) {
...
int update(MaterialRepo* v, Material m, Material n) {
	if (find(v, m.name, m.supplier, m.date) == -1)
		return 0;
	v->materials[find(v, m.name, m.supplier, m.date)] = n;
	return 1;
}
